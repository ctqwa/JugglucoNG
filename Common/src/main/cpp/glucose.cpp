...
void SensorGlucoseData::resetSiIndex() {
  ...
  // If useCustomCalibration is enabled, use the configured window
  if (getinfo()->useCustomCalibration) {
    int hours = getCustomCalHours(getinfo()->customCalIndex);
    // Sibionics samples every 5 mins -> 12 samples/hour
    int windowSize = hours * 12;

    int newIndex = currentIndex - windowSize;
    if (newIndex < 1)
      newIndex = 1;

    LOGGER("resetSiIndex (Custom): current=%d hoursWindow=%d windowSize=%d "
           "newIndex=%d\n",
           currentIndex, hours, windowSize, newIndex);
    setSiIndex(newIndex);
  } else {
