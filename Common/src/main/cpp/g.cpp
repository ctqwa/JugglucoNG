...
#ifdef SIBIONICS
std::recursive_mutex si_lifecycle_mutex;
...
extern "C" JNIEXPORT jlong JNICALL fromjava(getdataptr)(JNIEnv *env, jclass cl,
  ...
  if (sens->isSibionics()) {
    ...
    sistream *s = new sistream(sensorindex, sens);
    {
      std::lock_guard<std::recursive_mutex> lock(si_lifecycle_mutex);
      active_streams.insert(s);
    }
    data = s;
  } else
#endif
...
extern "C" JNIEXPORT void JNICALL fromjava(freedataptr)(JNIEnv *envin,
  ...
  if (sdata) {
#ifdef SIBIONICS
    if (sdata->libreversion == 0x10) {
      std::lock_guard<std::recursive_mutex> lock(si_lifecycle_mutex);
      if (active_streams.find(sdata) != active_streams.end()) {
        active_streams.erase(sdata);
        delete static_cast<sistream *>(sdata);
      }
      return;
    }
#endif
