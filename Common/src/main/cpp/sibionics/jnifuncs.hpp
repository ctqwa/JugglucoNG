...
AlgorithmContext *vers(initAlgorithm)(SensorGlucoseData *sens,
  ...
  if (sens->getSiIndex() > 1) {
    ...
    if (saved) {
      ...
      // Empty binState with siIndex > 1: algorithm starts fresh
      // Do NOT auto-reset here - only SiContext::reset() should trigger
      // recalibration
      LOGGER("binState empty, siIndex=%d - algorithm starting fresh\n",
             sens->getSiIndex());
    }
  }
