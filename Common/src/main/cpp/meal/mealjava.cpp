...
fromjava(saveingredient)(JNIEnv *env, jclass thiz, jint pos, jstring jname,
                         jstring junit, jfloat carbo) {
...

  {
    ...
    if (rawlen <= maxnamelen ||
        (jnamelen == rawlen && (jnamelen = maxnamelen))) {
  ...
  if (junitlen <= maxunitlen ||
      (rawunitlen == junitlen && (junitlen = maxunitlen))) {
  ...
  auto unit_it = std::find_if(units.begin(), &units[unitnr],
                              [newunit](ingredientunit_t &one) {
...
fromjava(ingredientdeleteable)(JNIEnv *env, jclass thiz, jint pos) {
  return meals->datameal()->deleteable(pos);
}
extern "C" JNIEXPORT void JNICALL fromjava(deleteingredient)(JNIEnv *env,
                                                             jclass thiz,
                                                             jint pos) {
...
extern "C" JNIEXPORT jint JNICALL fromjava(ingredientNr)(JNIEnv *env,
                                                         jclass thiz) {
  return meals->datameal()->ingredientnr;
}
extern "C" JNIEXPORT jfloat JNICALL fromjava(ingredientCarb)(JNIEnv *env,
                                                             jclass thiz,
                                                             jint index) {
  return meals->getingredients()[index].carb;
}
extern "C" JNIEXPORT jint JNICALL fromjava(ingredientUsed)(JNIEnv *env,
                                                           jclass thiz,
                                                           jint index) {
  return meals->getingredients()[index].used;
}
extern "C" JNIEXPORT jstring JNICALL fromjava(ingredientUnitName)(JNIEnv *env,
                                                                  jclass thiz,
                                                                  jint index) {
  return myNewStringUTF(
      env, meals->getunits()[meals->getingredients()[index].unit].data());
}
extern "C" JNIEXPORT jint JNICALL fromjava(ingredientUnit)(JNIEnv *env,
                                                           jclass thiz,
                                                           jint index) {
  return meals->getingredients()[index].unit;
}
/*
extern "C" JNIEXPORT jbyteArray JNICALL fromjava(ingredientNameBytes)(JNIEnv
*env, jclass thiz,jint index) { const char
*name=meals->getingredients()[index].name.data(); const int len=strlen(name)+1;
    jbyteArray uit=env->NewByteArray(len);
    env->SetByteArrayRegion(uit, 0, len,(const jbyte*)name);
   return uit;
    }
    */
extern "C" JNIEXPORT jstring JNICALL fromjava(ingredientName)(JNIEnv *env,
                                                              jclass thiz,
                                                              jint index) {
...
extern "C" JNIEXPORT jint JNICALL fromjava(changemealitem)(JNIEnv *env,
                                                           jclass thiz,
                                                           jint mealptr,
                                                           jint pos, jint ingre,
                                                           jfloat amount) {
...
extern "C" JNIEXPORT jint JNICALL fromjava(additemtomeal)(
    JNIEnv *env, jclass thiz, jint mealptr, jint ingre, jfloat amount) {
  return meals->datameal()->additemtomeal(mealptr, ingre, amount);
}
extern "C" JNIEXPORT jfloat JNICALL fromjava(getitemamount)(JNIEnv *env,
                                                            jclass thiz,
                                                            jint mealptr,
                                                            jint pos) {
  return meals->datameal()->getitemamount(mealptr, pos);
}
extern "C" JNIEXPORT jstring JNICALL fromjava(getitemingredientname)(
    JNIEnv *env, jclass thiz, jint mealptr, jint pos) {
  const char *ingr = meals->datameal()->getitemingredientname(mealptr, pos);
  if (ingr)
    return myNewStringUTF(env, ingr);
  return nullptr;
}
extern "C" JNIEXPORT jint JNICALL fromjava(getitemingredient)(JNIEnv *env,
                                                              jclass thiz,
                                                              jint mealptr,
                                                              jint pos) {
  return meals->datameal()->getitemingredient(mealptr, pos);
}
extern "C" JNIEXPORT jint JNICALL fromjava(getmealitemnr)(JNIEnv *env,
                                                          jclass thiz,
                                                          jint mealptr) {
  return meals->datameal()->itemsinmeal(mealptr);
}
extern "C" JNIEXPORT jint JNICALL fromjava(cpmeal)(JNIEnv *env, jclass thiz,
                                                   jint mealptr) {
  return meals->datameal()->cpmeal(mealptr);
}
extern "C" JNIEXPORT void JNICALL fromjava(deletemeal)(JNIEnv *env, jclass thiz,
                                                       jint mealptr) {
  meals->datameal()->deletemeal(mealptr);
}
extern "C" JNIEXPORT jfloat JNICALL fromjava(carbinmeal)(JNIEnv *env,
                                                         jclass thiz,
                                                         jint mealptr) {
  return meals->datameal()->carbinmeal(mealptr);
}
extern "C" JNIEXPORT jint JNICALL fromjava(getnewmealptr)(JNIEnv *env,
                                                          jclass thiz) {
...
extern "C" JNIEXPORT void JNICALL fromjava(closemeal)(JNIEnv *env, jclass thiz,
                                                      jint mealptr) {
  meals->datameal()->endmeal(mealptr);
}
extern "C" JNIEXPORT jint JNICALL fromjava(deletefrommeal)(JNIEnv *env,
                                                           jclass thiz,
                                                           jint mealptr,
                                                           jint pos) {
...
extern "C" JNIEXPORT jobject JNICALL fromjava(getunits)(JNIEnv *env,
                                                        jclass cl) {
  ...
  for (int i = 0; i < len; i++) {
    env->CallBooleanMethod(
        arlist, add, myNewStringUTF(env, meals->datameal()->units[i].data()));
  }
  return arlist;
}
...
fromjava(searchIngredient)(JNIEnv *env, jclass cl, jstring jlabel) {
  ...
  if (reg_ingredient(label, vect, meals->getingredients().data(),
                     meals->datameal()->ingredientnr)) {
